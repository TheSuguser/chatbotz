...
from preprocess import sentence_cut, sentence_to_vec, load_word_vec, get_similarity_df, get_answer, get_random_question
...
while True: 
    ...
    if if_next == 1:
        ...
        print(answer)
        response_next = input()
        if re.sub(r'[^\w\s]','',response_next) in ['是的','是']:
            answer, similarity = get_answer(response, df_qa, word_vec)
            if similarity >=0.9:
                print(answer)
            else:
        ...
        elif re.sub(r'[^\w\s]','',response_next) in ['不是的','不是','不']:
            print('那麻烦您重新提问。')
        else:
            status = 1
    else:
